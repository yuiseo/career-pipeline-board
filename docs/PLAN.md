...
| 완료 | 커밋 | 체크리스트 |
|---|---|---|
| [x] | `test(mock-api)` | [x] 목록 응답에 요약 필드만 있음 [x] 상세 응답에 상세 필드 포함 [x] PATCH 결과가 저장소에 반영됨 [x] `failRate: 1`이면 500 [x] 저장소 버전이 다르면 시드 재생성 |
...
| 완료 | 커밋 | 내용 |
|---|---|---|
| [x] | `feat(race-condition)` | `pendingTarget` 추가, 지원자별 요청 직렬화, 요청 중 이동 메뉴 활성화, DECISIONS 2-3 시나리오 2~7과 세부 규칙 적용 |
| [ ] | `feat(virtualization)` | 1,000건 기준 컬럼별 가상 스크롤 (라이브러리는 이 단계에서 확정) |
| [x] | `feat(undo)` | 방금 한 이동 취소. 되돌리기도 같은 이동 흐름(새 이동 의도)으로 처리. 상세 Undo 후 「이동 중」고착은 후속 fix |
